...
Eigen::MatrixXi MeshCutter::reindex_feature_edges(const Eigen::MatrixXi& FE) {
	...
	if (feature_edges.size() != FE.rows())
	{
		spdlog::error("Failure in reidnexing, an edge was not found in any face");
		exit(EXIT_FAILURE);
	}
...
Eigen::MatrixXi MeshCutter::remove_cycles_and_duplicates(const Eigen::MatrixXi& FE, const Eigen::MatrixXi& FE_reindex) {
	...
	for (int i = 0; i < FE.rows(); ++i)
	{
		...
		if (uf.find_set(first) != uf.find_set(second)) {
            ...
            FE_new(r, 0) = FE_reindex(i, 0);
            FE_new(r, 1) = FE_reindex(i, 1);
            FE_new(r, 2) = alignment;
            ++r;
        }
        else {
