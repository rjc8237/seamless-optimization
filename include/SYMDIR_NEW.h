...
// #define TEST_AMIPS
#ifndef TEST_AMIPS
        return frob2 * (1 + 1 / (det * det)); // Symmetric Dirichlet
#else
        return (a2 + b2 + c2 + d2) / det; // Conformal AMIPS 2D
#endif
    }
    ...
    // a = uv coords
    template <typename Scalar>
    Scalar symmetric_dirichlet_energy(
        const Eigen::Vector<Scalar, 3>& A,
        const Eigen::Vector<Scalar, 3>& B,
        const Eigen::Vector<Scalar, 3>& C,
        const Eigen::Vector<Scalar, 2>& a,
...
#define AVOID_ROTATION_ERROR
    ...
    double get_grad_and_hessian_onering(
        ExtremeOpt& m,
        Tuple t,
        Eigen::Matrix<double, 2, 1>& grad,
        Eigen::Matrix2<double>& hessian)
    {
        grad.setZero();
        hessian.setZero();
        ...
        DScalar E;
        switch (energy_type) {
        case EnergyType::Lp: E = Lp(areas, energy); break;
        case EnergyType::SoftMax: E = SoftMax(energy); break;
        }
    ...
    double symmetric_dirichlet_energy_onering(
        ExtremeOpt& m,
        Tuple t)
    {
        double E = 0;
        ...
        case EnergyType::Lp: E = Lp(areas, energy); break;
        case EnergyType::SoftMax: E = SoftMax(energy); break;
        case EnergyType::Max: E = energy.maxCoeff(); break;
        }
        return E;
    }

    double symmetric_dirichlet_energy_2chart(
        ExtremeOpt& m,
        Tuple t)
    {
        double E = 0;
...
private:
    ...
    void get_ordered_abc(
        ExtremeOpt& m,
        Tuple& t,
        Tuple& loc,
        Eigen::Vector<double, 3>& A,
